...
<body class="bg_radial_2">
  ...
  echo "</div></section>";
  ?>
  <p class="text-white h3 mx-3">Feedbacks</p>
  <div class="container_md m-3 p-3">
    <?php
    include("connection.php");
    if (mysqli_num_rows($result_feedbacks) == 0) {
      echo "<p class=\"text-secondary\"> No feedback available</p>";
    } else {
      while ($r = mysqli_fetch_array($result_feedbacks)) {
        $customer_query = "SELECT * FROM users WHERE user_id='" . $r["customer_id"] . "';";
        $result_customer = mysqli_query($conn, $customer_query);
        $row_customer = mysqli_fetch_array($result_customer);
        echo "
      <div class=\"container-md text-white border border-2 border-white my-2 py-1 px-2\">
        <h5 class=\"d-inline-block p-1 m-0\"style=\"background-color:rgba(225, 135, 255, 1) \">
          " . $row_customer["user_name"] . "
        </h5>
        <p class=\" font-italic font-weight-bold p-0 m-0\">
          " . $row_customer["user_email"] . "
        </p>
        <h6 class=\" font-weight-light p-0 m-0\">
          " . $r["feedback_description"] . "
        </h6>";
        $a = $r["feedback_rate"];
        if ($a == 0) {
    ...
    <div class="container text-center ">
      <span class="me-1"><a class="p-0 m-0 text-decoration-none" <?php echo "href=\"service.php?sid=" . $sid . "\""; ?>>
          <button class=" btn btn-gold btn-md m-0">
            View Service
          </button>
        </a>
      </span>
      <span>
        <a class="p-0 m-0 text-decoration-none" <?php echo "href=\"add_feedbacks.php?sid=" . $sid . "\""; ?>>
          <button class=" btn btn-gold btn-md m-0">
            Add Feedback
          </button>
        </a>
